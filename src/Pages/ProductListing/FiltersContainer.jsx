...
const FiltersContainer = (props) => {
  ...
  useEffect(() => {
    ...
    if (data?.categories?.length && ctgrySelected.ctgryId !== 0) {
      let fltrArr = data.categories.filter(
        (el) => el.id == ctgrySelected.ctgryId
      );
      fltrArr[0].productTags.map((cVal, cIndx) => {
        setPrdctTags((prevState) => [
          ...prevState,
          { id: cIndx, prdctTagTile: cVal, isSelected: false },
        ]);
      });
      fltrArr[0].brandfilters.map((cVal, cIndx) => {
        setBrands((prevState) => [
          ...prevState,
          { id: cIndx, brandName: cVal, isSelected: false },
        ]);
      });
    }
  }, [data, ctgrySelected]);
  ...
  useEffect(() => {
    ...
    if (Object.keys(props.categry).length && data?.categories?.length) {
      const indx = data?.categories?.findIndex(
        (el) => el.title === props.categry.categoryName
      );
      setCtgrySelected({
        indx: indx,
        ctgryId: Number(props.categry.categoryId),
      });
    }
  ...
  const handleProductsTagCheckBox = (prdctTag) => {
    ...
    const selectedProductTags = prdctTagsArr
      .filter((el) => el.isSelected === true)
      .reduce((acc, cVal) => [...acc, cVal.prdctTagTile], []);
  ...
  const handleBrandsCheckBox = (brandName) => {
    ...
    setBrands([...brandsArr]);
    const selectedBrands = brandsArr
      .filter((el) => el.isSelected === true)
      .reduce((acc, cVal) => [...acc, cVal.brandName], []);
            ...
            <div className="col-12 brands-container">
              {brands.map((cVal, cIndx) => {
                return (
                  <div
                    key={cIndx}
                    className="col-12 prdct-tag-filter-container"
                  >
                    <Input
                      className="checkbox"
                      type="checkbox"
                    ...
                    />
                    <label className="text-xs lbl-prdct-tag">
                      {cVal.brandName}
                    </label>
                ...
                return (
                  ...
                  >
                    <Input
                      ...
                      onChange={() =>
                        handleProductsTagCheckBox(cVal.prdctTagTile)
                      }
                      checked={cVal.isSelected}
                    />
                    <label className="text-xs lbl-prdct-tag">
      ...
      <div className="col-12 filters-container">
        <div className="price-filter">
          ...
          <Input
            className="radio-btn"
            type="radio"
            id="radiobtn-lowToHigh"
            name="lowToHigh"
            value="lowToHigh"
            onChange={() => dispatch({ type: "LOW_TO_HIGH" })}
            checked={sortBy === "LOW_TO_HIGH"}
          />
          <label className="lbl-price" for="lowToHigh">
            Low To High
          </label>
        </div>
        ...
        <div className="price-filter">
          <Input
            className="radio-btn"
            type="radio"
            id="radiobtn-HighTolow"
            name="HighTolow"
            value="HighTolow"
            onChange={() => dispatch({ type: "HIGH_TO_LOW" })}
            checked={sortBy === "HIGH_TO_LOW"}
          />
          <label className="lbl-price" for="HighTolow">
            High To Low
          </label>
        </div>
      </div>
          ...
          ? Object.keys(ratingsFilter).map((key) => (
              <div className="ratings-filter">
                <Input
                  ...
                  name="ratings"
                  value="ratings"
                  onChange={() => dispatch({ type: "RATINGS", payload: key })}
                  checked={ratings === key}
                />
